...
def purchased( request, uid, id ):
    ...
    if request.POST.has_key('txn_id'):
      txn = request.POST.get('txn_id')
      try:
        existing = models.Purchase.objects.get( tx=txn )
        return render_to_response('error.html', { 'error': "Duplicate transaction" }, context_instance=RequestContext(request) )
      except models.Purchase.DoesNotExist:
        result = paypal.Verify( txn,request )
        print result.result1
        print resource.price
        print result.amount 
        if result.result1 is "SUCCESS" and resource.price == result.amount: # valid
          purchase = models.Purchase( resource=resource, purchaser=user, tx=txn )
          purchase.save()
